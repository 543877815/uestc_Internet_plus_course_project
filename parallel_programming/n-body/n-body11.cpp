...
void bodyForce(Body *p, float dt, int n, int iter) {
  ...
  // share memory
  #pragma unroll
  for (unsigned int i = n / TOTAL_STREAM * iter; i < n / TOTAL_STREAM * (iter+1); i += BLOCK_SIZE){
...
int main(const int argc, const char** argv) {
   ...
   */
   ...
   bodyForce<<<nBlocks, MAX_BLOCK_SIZE>>>(d_p, dt, nBodies, 0); // compute interbody forces
   bodyForce<<<nBlocks, MAX_BLOCK_SIZE>>>(d_p, dt, nBodies, 1); // compute interbody forces
   bodyForce<<<nBlocks, MAX_BLOCK_SIZE>>>(d_p, dt, nBodies, 2); // compute interbody forces
   bodyForce<<<nBlocks, MAX_BLOCK_SIZE>>>(d_p, dt, nBodies, 3); // compute interbody forces
