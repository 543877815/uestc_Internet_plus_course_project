...
__global__ void bodyForce(Body *p, float dt, int n) {
  ...
  for (unsigned int i = 0; i < n; i+=32){
    float x_src = p[i + laneid].x;
    float y_src = p[i + laneid].y;
    float z_src = p[i + laneid].z;

    __syncthreads();
    #pragma unroll 32
    for (unsigned int j = 0; j < 32; j++){
      float dx = ( __shfl_sync(0xFFFFFFFF, x_src, j) - lgpx0 );
      float dy = ( __shfl_sync(0xFFFFFFFF, y_src, j) - lgpy0 );
      float dz = ( __shfl_sync(0xFFFFFFFF, z_src, j) - lgpz0 );
  ...
  p[index].x += p[index].vx*dt; p[index].y += p[index].vy*dt; p[index].z += p[index].vz*dt;
}
